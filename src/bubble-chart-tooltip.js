...
export function showTooltip(name, timeWatchedHours, timeWatchedMinutes, timeLeftHours, timeLeftMinutes)
{
    ...
    newDiv.id = "tooltip-bubble-chart"
    const currentDiv = document.getElementById("tooltip-bubble-chart");
    document.body.insertBefore(newDiv, currentDiv);
    document.getElementById('tooltip-bubble-chart').innerHTML = `
    <section class="tooltip-name">
        <h2>${name}</h2>
        <svg id="close-tooltip" height="20px" viewBox="0 0 512 512" width="20px" onclick="hideTooltip()" style="cursor:pointer;">
            <path fill="white"
